...
export function SolidBox({ id, isReference = false }: SolidBoxProps) {
  ...
  const [width, height, depth] = useMemo(() => {
    const dims = furnitureData.dimensions || [1, 10, 1]
    const w = typeof dims[0] === 'number' && !isNaN(dims[0]) ? Math.max(0.01, dims[0]) : 1
    const h = typeof dims[1] === 'number' && !isNaN(dims[1]) ? Math.max(0.01, dims[1]) : 10
    const d = typeof dims[2] === 'number' && !isNaN(dims[2]) ? Math.max(0.01, dims[2]) : 1
    return [w, h, d]
